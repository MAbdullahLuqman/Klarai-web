    ...
    <>
      ...
      <div className={`fixed top-4 md:top-6 left-0 w-full z-50 flex flex-col items-center px-4 transition-transform duration-500 ease-in-out ${isHidden && !isMobileMenuOpen ? '-translate-y-[150%]' : 'translate-y-0'}`}>
        ...
        <nav className="bg-white/85 border border-gray-200/50 shadow-sm backdrop-blur-xl rounded-full px-2 py-2 flex items-center justify-between w-full max-w-4xl relative z-50">
          ...
          {/* Desktop Links with Framer Motion Animated Box */}
          <div className="hidden md:flex items-center gap-1 font-mono text-[11px] font-bold tracking-[0.1em] uppercase text-gray-500">
            {navLinks.map((link) => (
              <Link key={link.path} href={link.path} className="relative px-4 py-2 transition-colors z-10 rounded-full">
                {isActive(link.path) && (
                  <motion.span
      ...
      {/* Note: z-40 so it sits perfectly behind the z-50 floating pill header */}
      <AnimatePresence>
        {isMobileMenuOpen && (
          <motion.div
          ...
          >
              ...
              {navLinks.map((link, i) => (
                ...
                >
                  ...
                  >
                    ...
                    {isActive(link.path) && (
                       <span className="w-3 h-3 rounded-full bg-[#ccff00] shadow-[0_0_10px_rgba(204,255,0,0.8)]" />
                    )}
                  </Link>
                </motion.div>
              ))}
            ...
            <motion.div 
              ...
              className="mt-auto pt-12 border-t border-gray-200 w-full"
            >
              <p className="text-xs font-bold uppercase tracking-widest text-gray-400 mb-4 text-center">Ready to Dominate?</p>
              <Link 
                href="/free-audit" 
                onClick={() => setIsMobileMenuOpen(false)}
                className="bg-[#ccff00] text-[#050505] flex items-center justify-center gap-2 py-5 rounded-2xl font-black uppercase tracking-widest text-sm shadow-[0_10px_30px_rgba(204,255,0,0.3)] active:scale-95 transition-transform"
              >
