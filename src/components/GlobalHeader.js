...
export default function GlobalHeader() {
  ...
  return (
    <div className={`fixed top-4 md:top-6 left-0 w-full z-50 flex flex-col items-center px-4 transition-transform duration-500 ease-in-out ${isHidden ? '-translate-y-[150%]' : 'translate-y-0'}`}>
      ...
      {/* FIXED: Removed large shadow, replaced with ultra-clean shadow-sm and border */}
      <nav className="bg-white/85 border border-gray-200/50 shadow-sm backdrop-blur-xl rounded-full px-2 py-2 flex items-center justify-between w-full max-w-4xl relative z-50">
       ...
       <Link href="/" onClick={() => setIsMobileMenuOpen(false)} className="flex items-center gap-2 pl-4 group pr-4 md:pr-8">
          ...
          {/* FIXED: Forced font-sans and explicit font-[900] weight to prevent changing across pages */}
          ...
          >
            <div className="bg-white/95 backdrop-blur-xl border border-gray-200/80 shadow-[0_20px_40px_rgba(0,0,0,0.1)] rounded-3xl p-6 flex flex-col gap-6">
              <div className="flex flex-col space-y-4">
                <Link href="/services" onClick={() => setIsMobileMenuOpen(false)} className="text-2xl font-black tracking-tighter text-gray-900 hover:text-blue-600 transition-colors border-b border-gray-100 pb-4">Services</Link>
                <Link href="/industries" onClick={() => setIsMobileMenuOpen(false)} className="text-2xl font-black tracking-tighter text-gray-900 hover:text-blue-600 transition-colors border-b border-gray-100 pb-4">Industries</Link>
                <Link href="/blog" onClick={() => setIsMobileMenuOpen(false)} className="text-2xl font-black tracking-tighter text-gray-900 hover:text-blue-600 transition-colors border-b border-gray-100 pb-4">Blog</Link>
              </div>
