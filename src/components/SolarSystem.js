...
const SERVICES_DATA = [
  { id: "aeo", orbitRadius: 22, angle: 0.5, orbitSpeed: 0.002, spinSpeed: 0.01, size: 3.5, color: "#1e40af", label: "AEO", type: "earthLike", title: "Answer Engine Optimization", teaser: "Click to explore AEO engineering & AI entity structuring." },
  { id: "seo", orbitRadius: 32, angle: 2.1, orbitSpeed: 0.0015, spinSpeed: 0.008, size: 4.2, color: "#94a3b8", label: "SEO", type: "ice", title: "Search Engine Optimization", teaser: "Click to explore organic dominance and high-intent ranking." },
  { id: "webdev", orbitRadius: 45, angle: 4.0, orbitSpeed: 0.001, spinSpeed: 0.02, size: 3.0, color: "#9a3412", label: "WEB", type: "terrestrial", title: "Web Development", teaser: "Click to explore high-performance 3D web architecture." },
  { id: "performance", orbitRadius: 60, angle: 1.2, orbitSpeed: 0.0008, spinSpeed: 0.005, size: 5.5, color: "#b45309", label: "ADS", type: "gasGiant", title: "Performance Marketing", teaser: "Click to explore paid scaling and algorithmic bidding." },
  { id: "smma", orbitRadius: 75, angle: 5.5, orbitSpeed: 0.0005, spinSpeed: 0.012, size: 4.0, color: "#d97706", label: "SMMA", type: "terrestrial", title: "Social Media Marketing", teaser: "Click to explore viral content and brand authority." }
];
...
function Planet({ data, onPlanetClick }) {
  ...
  return (
    <group 
      ref={orbitGroupRef}
      frustumCulled={false} 
      onClick={(e) => { e.stopPropagation(); if (typeof onPlanetClick === 'function') onPlanetClick(data); }}
      onPointerOver={() => { setHovered(true); document.body.style.cursor = 'pointer'; }}
      onPointerOut={() => { setHovered(false); document.body.style.cursor = 'auto'; }}
    >
      <group ref={planetMeshRef} frustumCulled={false}>
        <Sphere args={[data.size, 64, 64]} frustumCulled={false}>
          <meshPhysicalMaterial color={data.color} metalness={0.4} roughness={0.8} clearcoat={0.2} clearcoatRoughness={0.5} emissive={data.color} emissiveIntensity={hovered ? 0.3 : 0.05} />
        </Sphere>
        ...
        <Text position={[0, 0, data.size + 0.1]} fontSize={data.size * 0.35} color="white" fontWeight="bold" anchorX="center" anchorY="middle">{data.label}</Text>
        <Text position={[0, 0, -(data.size + 0.1)]} rotation={[0, Math.PI, 0]} fontSize={data.size * 0.35} color="white" fontWeight="bold" anchorX="center" anchorY="middle">{data.label}</Text>
      </group>
    </group>
  );
}
...
function CameraFlyer({ isStrategyModalOpen }) {
  ...
  useFrame((state) => {
    if (isStrategyModalOpen) {
      ...
      const radius = 130; 
      const targetX = Math.cos(time * 0.15) * radius; 
      ...
      <Canvas camera={{ position: [0, 25, 130], fov: 60 }}>
        ...
        <ScrollControls pages={10} damping={0.2}>
          <group 
            position={[0, 0, 0]} 
            frustumCulled={false}
            onClick={(e) => { e.stopPropagation(); if (typeof onSunClick === 'function') onSunClick(); }}
          ...
          >
            <Sphere args={[8.0, 64, 64]} frustumCulled={false}><meshBasicMaterial color="#fffbeb" /></Sphere>
            <Sphere args={[9.5, 64, 64]} frustumCulled={false}><meshBasicMaterial color="#f59e0b" transparent={true} opacity={0.6} blending={THREE.AdditiveBlending} depthWrite={false} /></Sphere>
            <Sphere args={[12.5, 32, 32]} frustumCulled={false}><meshBasicMaterial color="#ea580c" transparent={true} opacity={sunHovered ? 0.25 : 0.15} blending={THREE.AdditiveBlending} depthWrite={false} /></Sphere>
          ...
          {SERVICES_DATA.map((service) => <Planet key={service.id} data={service} onPlanetClick={onPlanetClick} />)}
