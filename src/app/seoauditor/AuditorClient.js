  ...
  const downloadPDF = async () => {
    ...
    try {
      const response = await fetch('/api/generate-pdf', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ url: formatUrl(url), auditResult })
      });
      ...
      {auditComplete && auditResult && (
        <section className="w-full bg-[#fafafa] py-12 px-4 sm:px-6">
          <div ref={reportRef} className="w-full max-w-[1000px] mx-auto animate-fade-in relative z-10 space-y-5 pb-10">
             ...
             {/* BOTTOM CTA STRIP */}
             <div className="mt-6 flex flex-col sm:flex-row items-center justify-between gap-3 bg-white p-4 rounded-[1rem] border border-gray-200 shadow-md text-center sm:text-left">
                <div>
