...
export default function CleanAuditGateway() {
  ...
  const inputClass = "w-full bg-transparent border-b border-gray-700 hover:border-gray-500 py-3 text-white text-xl sm:text-2xl text-center focus:outline-none focus:border-[#008dd8] transition-all placeholder:text-gray-700 font-light";
  const labelClass = "text-[10px] font-black text-gray-500 uppercase tracking-[0.2em] block mb-4 transition-colors group-focus-within:text-[#008dd8] text-center";

  return (
    <main className="bg-[#030303] text-white font-sans selection:bg-[#008dd8] selection:text-white flex flex-col">
      <GlobalHeader />
      
      {/* --- HERO: CENTERED MULTI-STEP MODAL --- */}
      <section className="relative w-full pt-32 pb-24 px-4 sm:px-6 flex items-center justify-center min-h-screen border-b border-white/5 overflow-hidden">
        
        {/* Ambient Page Glow */}
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[80vw] h-[80vw] bg-[#008dd8]/[0.02] blur-[150px] rounded-full pointer-events-none"></div>

        {/* Centered Modal Container */}
        <div className="w-full max-w-[800px] bg-[#0A101D]/90 backdrop-blur-2xl border border-white/10 rounded-3xl shadow-2xl flex flex-col min-h-[500px] relative overflow-hidden">
          
          {/* Progress Bar */}
          <div className="absolute top-0 left-0 w-full h-1 bg-white/5 z-20">
            <motion.div 
              className="h-full bg-gradient-to-r from-[#008dd8] to-[#00b4d8] shadow-[0_0_10px_#008dd8]"
              initial={{ width: '0%' }}
          ...
          <div className="w-full h-full p-8 sm:p-12 md:p-16 relative flex flex-col justify-center flex-grow">
            ...
            {isSuccess ? (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex flex-col items-center text-center">
                <div className="w-20 h-20 bg-green-500/10 border border-green-500/20 text-green-500 rounded-full flex items-center justify-center mb-6 shadow-[0_0_30px_rgba(34,197,94,0.1)]">
                  <svg className="w-10 h-10" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M5 13l4 4L19 7"></path></svg>
                </div>
                <h2 className="text-3xl font-black text-white mb-2">Request Secured.</h2>
                <p className="text-gray-400">Our engineering team has received your details, {firstName}.</p>
              </motion.div>
            ) : (
              <form onSubmit={handleNext} className="w-full h-full flex flex-col">
                <div className="flex-grow flex flex-col justify-center items-center">
                  <AnimatePresence mode="wait">
                    ...
                    >
                      ...
                      {step === 1 && (
                        <>
                          <h2 className="text-3xl sm:text-5xl font-black text-white mb-12 tracking-tight text-center">Let's build together.</h2>
                      ...
                      {step === 2 && (
                        <>
                          <h2 className="text-3xl sm:text-4xl font-black text-white mb-10 tracking-tight text-center">Nice to meet you, {firstName}.</h2>
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                              {['Free Technical SEO Audit', 'AI Answer Engine Optimization', 'Web Architecture', 'General Inquiry'].map((srv) => (
                                <button
                                  ...
                                  className={`p-4 rounded-xl border text-[10px] uppercase tracking-widest font-black transition-all duration-300 ${formData.service === srv ? 'bg-[#008dd8] border-[#008dd8] text-white shadow-[0_0_20px_rgba(0,141,216,0.3)]' : 'bg-transparent border-white/10 text-gray-500 hover:border-white/30 hover:text-white'}`}
                                >
                                  {srv}
                                </button>
                              ))}
                        ...
                        <>
                          ...
                          <div className="w-full max-w-lg group">
                            <label className={labelClass}>
                              Website URL {formData.service === 'Free Technical SEO Audit' && <span className="text-[#008dd8]">(Required)</span>}
                            </label>
                      ...
                      {step === 4 && (
                        <>
                          <h2 className="text-3xl sm:text-4xl font-black text-white mb-10 tracking-tight text-center">Comm channel.</h2>
                      ...
                      {step === 5 && (
                        <>
                          <h2 className="text-3xl sm:text-4xl font-black text-white mb-12 tracking-tight text-center">Final parameters.</h2>
                ...
                {/* BOTTOM NAVIGATION */}
                <div className="pt-10 flex justify-between items-center mt-8 relative z-20 w-full max-w-2xl mx-auto border-t border-white/5">
                  <div className="w-1/3">
                    {step > 1 && (
                      <button type="button" onClick={handlePrev} className="text-gray-500 hover:text-white text-[10px] font-black uppercase tracking-widest transition-colors flex items-center gap-2">
                        ← Back
                      </button>
                    )}
                  </div>
                  <div className="w-1/3 text-center text-[10px] text-gray-600 font-mono tracking-widest">
                    0{step} / 0{totalSteps}
                  </div>
                  <div className="w-1/3 flex justify-end">
                    <button type="submit" disabled={isSubmitting} className="bg-white hover:bg-gray-200 text-black px-6 py-3 rounded-full font-black text-[10px] uppercase tracking-widest transition-all active:scale-95 disabled:opacity-50 flex items-center gap-2">
      ...
      <section className="w-full py-24 px-6 border-b border-white/5">
        <div className="max-w-[800px] mx-auto">
          <div className="bg-[#008dd8]/10 border-l-2 border-[#008dd8] p-6 rounded-r-xl mb-12">
          ...
          <h2 className="text-3xl font-black text-white mb-6">Most websites have SEO problems hiding in plain sight.</h2>
      ...
      <section className="w-full py-24 px-6 border-b border-white/5 bg-[#0A101D]/50">
        <div className="max-w-[1000px] mx-auto">
          <div className="text-center mb-16">
            <h2 className="text-3xl md:text-4xl font-black text-white mb-4">What makes this SEO auditor different?</h2>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div className="bg-white/5 p-8 rounded-2xl border border-white/10">
              <h3 className="text-sm font-black text-gray-400 mb-6 uppercase tracking-widest">Standard Free Checkers</h3>
              <ul className="space-y-4">
                {["Scan one page in isolation", "Return a score with no priority ranking", "Generic advice that applies to every site", "No AI visibility or GEO checks"].map((item, i) => (
                  <li key={i} className="flex items-start gap-3 text-sm text-gray-500 font-light"><span className="text-gray-600 mt-0.5">✖</span> {item}</li>
                ))}
              </ul>
            </div>
            <div className="bg-gradient-to-br from-[#008dd8]/20 to-transparent p-8 rounded-2xl border border-[#008dd8]/30">
              <h3 className="text-sm font-black text-white mb-6 uppercase tracking-widest text-[#008dd8]">Klarai SEO Auditor</h3>
              <ul className="space-y-4">
                {["Gemini-powered analysis in 30 seconds", "Findings prioritised by revenue impact", "Specific, actionable recommendations", "AI Overview eligibility checks built in"].map((item, i) => (
                  <li key={i} className="flex items-start gap-3 text-sm text-gray-200 font-medium"><span className="text-[#008dd8] mt-0.5">✔</span> {item}</li>
                ))}
              </ul>
            </div>
          </div>
      ...
      <section className="w-full py-24 px-6 border-b border-white/5">
        <div className="max-w-[1200px] mx-auto">
          <div className="text-center mb-16">
            <h2 className="text-3xl md:text-4xl font-black text-white mb-4">What the Klarai auditor checks</h2>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {[
              ...
              { num: "06", title: "AI Readiness", desc: "LLM accessibility and entity clarity for ChatGPT & Perplexity." }
               ...
               <div key={i} className="bg-white/5 p-8 rounded-2xl border border-white/10 hover:border-[#008dd8]/50 transition-colors group">
                 <span className="text-2xl font-black text-white/10 group-hover:text-[#008dd8]/40 transition-colors block mb-4">{item.num}</span>
      ...
      <section className="w-full py-24 px-6 bg-[#0A101D]/30">
        <div className="max-w-[800px] mx-auto">
          ...
          <div className="space-y-4">
            {[
              ...
              <div key={i} className="bg-white/5 border border-white/10 rounded-2xl p-6">
                <h4 className="text-sm font-black text-white mb-2">{faq.q}</h4>
