...
export async function generateMetadata({ params }) {
  const resolvedParams = await params; // FIXED: Awaited params
...
export default async function NicheLandingPage({ params }) {
  ...
  // --- UI/UX DEFINITIONS FOR READABILITY ---
  const brutalityH2 = "text-4xl md:text-5xl font-black tracking-tighter uppercase leading-tight mb-8 text-left";
      ...
      <main className="pb-16 md:pb-24">
        ...
        <section className="relative px-6 pt-[120px] md:pt-[140px] pb-16 md:pb-24 border-b border-gray-200 overflow-hidden">
          ...
          <div className="max-w-4xl mx-auto relative z-10 text-left">
            ...
            <h1 className="text-5xl md:text-7xl leading-[0.95] font-black tracking-tighter uppercase text-gray-900 mb-8 text-left">
              {page.h1 || 'Dominant Search Architecture'}
            </h1>
            <div className="text-xl md:text-2xl text-gray-600 font-medium leading-relaxed mb-10 text-left" dangerouslySetInnerHTML={{ __html: page.subheadline || '' }} />
            
            <div className="flex justify-center w-full">
              <Link href="/free-audit" className="inline-flex bg-[#ccff00] text-gray-900 px-12 py-6 rounded-full font-black text-sm md:text-lg uppercase tracking-widest hover:bg-black hover:text-white transition-all shadow-2xl active:scale-95 items-center gap-2">
        ...
        <div className="max-w-5xl mx-auto px-6 space-y-16 md:space-y-24 mt-16 md:mt-20">
          ...
          {page.tldr && (
            <section className="bg-white border-2 border-gray-100 p-8 md:p-12 rounded-[2rem] shadow-sm relative overflow-hidden">
              ...
              <h2 className="font-mono text-xs uppercase tracking-widest text-gray-400 font-bold mb-6 text-left">TL;DR / Executive Summary //</h2>
              <div className="text-xl md:text-2xl text-gray-800 font-bold leading-relaxed space-y-4 text-left" dangerouslySetInnerHTML={{ __html: page.tldr }} />
            </section>
          )}
          ...
          <section>
            <div className="mb-10 border-b border-gray-200 pb-8">
              ...
              <p className="text-gray-500 font-medium text-lg max-w-2xl text-left">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
              {(page.deliverables || []).map((item, i) => (
                <div key={i} className="bg-white p-8 border-2 border-gray-100 rounded-3xl hover:border-blue-500 transition-all group">
                  <div className="text-lg text-gray-800 font-medium leading-relaxed text-left" dangerouslySetInnerHTML={{ __html: item || '' }} />
                </div>
              ))}
            </div>
          </section>
          ...
          <section className="bg-gray-900 rounded-[2rem] md:rounded-[3rem] p-8 md:p-20 text-white relative overflow-hidden">
            ...
            <div className="relative z-10">
              <h2 className="text-4xl md:text-5xl font-black tracking-tighter uppercase mb-12 text-left">System Deployment</h2>
              <div className="space-y-6">
                {(page.steps || []).map((step, i) => (
                  <div key={i} className="flex flex-col md:flex-row md:items-center gap-6 bg-white/5 border border-white/10 p-6 md:p-8 rounded-3xl backdrop-blur-md">
                    ...
                    <div className="text-lg font-medium text-gray-200 leading-relaxed text-left" dangerouslySetInnerHTML={{ __html: step || '' }} />
                  </div>
                ))}
              </div>
            </div>
          ...
          {(diyPara || (diySteps && diySteps.length > 0) || articleContent) && (
            <section className="bg-white border-2 border-gray-100 p-8 md:p-16 rounded-[2rem] md:rounded-[3rem] shadow-sm">
              <div className="max-w-3xl mx-auto">
                <h2 className="text-4xl md:text-5xl font-black tracking-tighter uppercase leading-none mb-10 border-b-2 border-gray-100 pb-8 text-left">
                  The Blueprint.
                </h2>
                
                {diyPara && (
                  <div className="text-lg md:text-xl text-gray-700 font-medium leading-relaxed mb-10 text-left" dangerouslySetInnerHTML={{ __html: diyPara }} />
                )}
                  ...
                  <div className="space-y-8 mb-10">
                    {diySteps.map((step, i) => (
                      <div key={i} className="flex gap-5 items-start">
                        ...
                        <div className="text-lg text-gray-600 font-medium leading-relaxed pt-1 text-left" dangerouslySetInnerHTML={{ __html: step?.step || step || '' }} />
                      </div>
                    ))}
                  </div>
                )}
          ...
          {page.keywords && page.keywords.some(k => k.kw) && (
            <section>
              <h2 className="text-3xl font-black uppercase tracking-tighter mb-8 text-left">Target Intelligence</h2>
                    ...
                    <tbody className="divide-y-2 divide-gray-50">
                      {page.keywords.map((item, i) => item.kw ? (
                        <tr key={i} className="hover:bg-blue-50/50 transition-colors">
                          <td className="p-6 text-base font-bold text-gray-900 whitespace-nowrap text-left">{item.kw}</td>
          ...
          {page.faqs && page.faqs.some(faq => faq.q) && (
            <section>
              <h2 className="text-3xl font-black uppercase tracking-tighter mb-8 text-left">Common Queries</h2>
              <div className="space-y-4">
                {page.faqs.map((faq, i) => (
                  <div key={i} className="bg-white border-2 border-gray-100 p-8 rounded-3xl hover:border-blue-200 transition-all shadow-sm">
                    <h3 className="font-black text-gray-900 mb-3 text-lg md:text-xl text-left">Q: {faq?.q || ''}</h3>
                    <div className="text-base md:text-lg text-gray-600 font-medium leading-relaxed text-left" dangerouslySetInnerHTML={{ __html: faq?.a || '' }} />
                  </div>
                ))}
              </div>
            </section>
          ...
          {page.authorName && (
             <section className="bg-white border-2 border-gray-100 p-8 rounded-[2rem] shadow-sm flex flex-col md:flex-row items-start gap-8">
                <div className="w-20 h-20 bg-gray-100 rounded-3xl flex items-center justify-center font-black text-4xl text-gray-900 flex-shrink-0">
                    {page.authorName.charAt(0)}
                </div>
                <div className="flex-1 text-left w-full">
                    <h3 className="text-3xl font-black uppercase tracking-tighter mb-1">{page.authorName}</h3>
                    <p className="text-xs font-black uppercase tracking-widest text-gray-400 mb-5">{page.authorRole || 'Lead Architect'}</p>
                    <div className="text-lg text-gray-600 font-medium leading-relaxed space-y-4" dangerouslySetInnerHTML={{ __html: page.authorBio || '' }} />
                    <div className="mt-8 pt-6 border-t border-gray-100">
                        {/* FIXED TYPO HERE */}
                        <Link href={page.authorLinkedin || 'https://linkedin.com'} className="inline-flex items-center gap-2 text-sm font-black uppercase tracking-widest text-gray-900 hover:text-blue-600 transition-colors">
          ...
          {/* --- 9. FINAL CTA --- */}
          <section className="py-16 md:py-24 text-left border-t border-gray-200">
            <h2 className="text-5xl md:text-8xl font-black tracking-tighter uppercase leading-none mb-8 text-left">
              Ready to <br/> <span className="text-blue-600">Scale?</span>
            </h2>
            <div className="text-lg md:text-xl text-gray-600 font-medium mb-12 max-w-xl text-left leading-relaxed" dangerouslySetInnerHTML={{ __html: page.ctaText || '' }} />
            <Link href="/free-audit" className="inline-block bg-[#ccff00] text-gray-900 px-12 py-6 rounded-full font-black text-sm md:text-lg uppercase tracking-widest hover:bg-black hover:text-white transition-all shadow-2xl active:scale-95 flex items-center justify-center md:justify-start gap-2 w-full md:w-max">
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M13 10V3L4 14h7v7l9-11h-7z"></path></svg>
                Initiate Sequence
            </Link>
          </section>
