...
export async function generateMetadata({ params }) {
  ...
  const docSnap = await safeGetDoc(doc(db, 'niche_pages', slug), `niche_pages/${slug}`);
  if (!docSnap?.exists?.()) return { title: 'Page Not Found | Klarai' };
  return {
    title: docSnap.data()?.metaTitle || 'Klarai',
    description: docSnap.data()?.metaDescription || '',
    alternates: {
      canonical: canonical(`/niche/${slug}`),
    },
  };
}
...
export default async function NicheLandingPage({ params }) {
  ...
  try {
    ...
    const blogSnap = await safeGetDocs(blogQ, 'blog_posts related');
